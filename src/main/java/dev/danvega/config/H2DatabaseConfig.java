 ...
 * 
 * <p>
 * The password is set AFTER Liquibase migrations run by executing
 * ALTER USER SA SET PASSWORD. This ensures:
 * <ul>
 * <li>Liquibase can connect with no password during startup</li>
...
public class H2DatabaseConfig {
    ...
    /**
     * ApplicationRunner that sets the SA password after Liquibase runs.
     * Order 100 ensures this runs after Liquibase (which typically runs at order
     * 0).
     */
    @Bean
    @Order(100)
    public ApplicationRunner h2PasswordInitializer(DataSource dataSource) {
        return args -> {
            ...
            try (Connection conn = dataSource.getConnection();
                ...
                logger.warn("Could not set H2 password: {}. Using default (empty) password.", e.getMessage());
    ...
    private void logDatabaseCredentials() {
        if (logger.isInfoEnabled() && generatedPassword != null) {
            String passwordPadded = padRight(generatedPassword, 30);
