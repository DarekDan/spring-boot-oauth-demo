 ...
 *
 * <p>
 * The password is set AFTER Liquibase migrations run by executing ALTER USER SA
 * SET PASSWORD.
 * This ensures:
 *
 * <ul>
...
public class H2DatabaseConfig {
    ...
    /**
     * ApplicationRunner that sets the SA password after Liquibase runs. Order 100
     * ensures this runs
    ...
    private void logDatabaseCredentials() {
        String passwordPadded = String.format("%-25s", generatedPassword);

        logger.info("╔═══════════════════════════════════════════════════════════════╗");
        logger.info("║            AUTHORIZATION SERVICE - H2 DATABASE                ║");
        logger.info("╠═══════════════════════════════════════════════════════════════╣");
        logger.info("║  Console URL:  http://localhost:8081/h2-console               ║");
        ...
        logger.info("║  Also available: readonly/readonly (SELECT-only access)       ║");
        logger.info("╚═══════════════════════════════════════════════════════════════╝");
    }
